...
/**
 * Reference values in this file come from two independent sources, the
 * same discipline Skin Diary's stats.test.ts uses (this suite is adapted
 * from it — see the Canopy README for the credit):
 *
 *  1. Closed-form identities (Γ(n) = (n-1)!, the Cauchy CDF, the
 *     arcsine form of I_x(½,½), I_x(1,1) = x).
 *  2. Hand-computable examples and textbook reference numbers, so the
 *     arithmetic can be checked by a reader without running the code.
 */
...
describe("studentTTwoTailed", () => {
  it("matches known reference p-values", () => {
...
describe("pearson", () => {
  ...
  it("returns NaN when a series has zero variance", () => {
    // A flat series (e.g. every entry logged the same mood) has no
    // correlation *defined* -- reporting 0 would be a claim we can't
    // support.
...
describe("spearman", () => {
  ...
  it("is 1 for any monotone increasing relationship, even a nonlinear one", () => {
    ...
    // Pearson does NOT see this as perfect -- which is exactly why Canopy
    // (like Skin Diary before it) defaults to Spearman for self-reported
    // diary data.
    expect(pearson(x, y)).toBeLessThan(0.95);
  });

  it("resists a wild outlier that Pearson cannot", () => {
    // One epic 500-minute hike day should not let a single outlier
    // dominate the read on "does outdoor time track with mood".
...
describe("correlationTest", () => {
  ...
  it("reports a weak correlation over few points as non-significant", () => {
    // r = 0.5 over 5 logged days is nothing -- p must be nowhere near 0.05.
...
describe("benjaminiHochberg", () => {
  ...
  it("penalises exactly as many tests as were actually run", () => {
    // Canopy runs a family of six hypotheses per diary (2 metrics x 2
    // lags, plus 2 group comparisons); this checks the correction scales
    // with family size the way the six-hypothesis family actually needs.
    const alone = benjaminiHochberg([0.02])[0];
    const amongMany = benjaminiHochberg([0.02, ...Array(5).fill(0.4)])[0];
...
describe("classifyConfidence — the honesty gate", () => {
  it("refuses to speak below the minimum sample size, however big r looks", () => {
    // r = 0.95 over 5 logged days is noise, and must be labelled as such
  ...
  it("grades surviving signals by effect size", () => {
    ...
    expect(classifyConfidence(0.2, 30, 0.02)).toBe("weak");
  });

  it("a small-n claim never outranks a large-n claim of the same shape", () => {
    // The acceptance criterion in prose: n=3 must read weaker than n=30.
    expect(classifyConfidence(0.7, 3, 0.001, 8)).toBe("insufficient");
    expect(classifyConfidence(0.7, 30, 0.001, 8)).toBe("strong");
  });
});
