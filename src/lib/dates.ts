 ...
 *
 * Adapted from Skin Diary (`~/Projects/skin-diary/src/lib/dates.ts`) — this
 * file is generic calendar arithmetic with no skin-specific content, so it
 * is reused close to verbatim.
 *
 * Everything in Canopy keys off a local YYYY-MM-DD string rather than a
 * timestamp. A diary is a thing you fill in "today", and timezone
 * arithmetic on Date objects is a reliable source of off-by-one-day bugs
 * that would silently misalign an outdoor-time-vs-mood lag comparison.
 */
