 ...
 *
 * Adapted verbatim from Skin Diary
 * (`~/Projects/skin-diary/src/lib/stats/distributions.ts`) — this is
 * general-purpose closed-form distribution math with nothing skin-specific
 * in it, so it is reused as-is rather than reinvented. See the Canopy
 * README for the full credit.
 *
 * Hand-rolled rather than pulled from a library so the whole inference
 * path is auditable in-repo, and so every p-value Canopy prints can be
 * traced to code that is unit-tested against known reference values
 * (closed-form identities and an independent implementation).
 *
